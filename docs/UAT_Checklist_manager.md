...
### Test 6A: The One-Prompt Analysis-to-Visualization Check

- Action:
  Prompt the Copilot:
  `Run a 1-way sweep on Gender, then generate a visualization.`
- Expected Pass:
  The Orchestrator runs the Actuary step first and then immediately routes into the Analyst step in the same request.
  The final response should include the actuarial summary first and the visualization/report result after it.
  A visualization artifact should be created from the fresh sweep output without requiring `continue`.
